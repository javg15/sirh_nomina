...
import { Recibos, Personal, Catquincena, Catrecibosestatus, Catrecibostipos, Catfondospresupuestales} from '../../../../_models';
import { RecibosService } from '../services/recibos.service';
import { CatrecibostiposService } from '../../../catalogos/catrecibostipos/services/catrecibostipos.service';
import { CatrecibosestatusService } from '../../../catalogos/catrecibosestatus/services/catrecibosestatus.service';
import { CatfondospresupuestalesService } from '../../../catalogos/catfondospresupuestales/services/catfondospresupuestales.service';
...
@Component({
  ...
  styleUrls: ['./recibos-form.component.css']
})

export class RecibosFormComponent implements OnInit, OnDestroy {
  userFormIsPending: Observable<boolean>; //Procesando información en el servidor
  @Input() id: string; //idModal
  @Input() botonAccion: string; //texto del boton según acción
  @Output() redrawEvent = new EventEmitter<any>();

  nombreModulo = 'Recibos';
  ...
  @ViewChild('id_personal_beneficiario') id_personal_beneficiario:AutocompleteComponent;
  @ViewChild('basicModalRecibos') basicModalRecibos: ModalDirective;
  @ViewChild('successModal') public successModal: ModalDirective;
  @ViewChild(ValidationSummaryComponent) validSummary: ValidationSummaryComponent;

  record:Recibos={
    id: 0, id_personal: 0, numero: "", fecha: null, anio: '', id_catrecibosestatus: 0,
    id_catrecibostipos: 0, id_catfondopresupuestal: 0, 
    ...
    fecharealpago: null, recibosustitucion: 0, recibotimbrado: 0, aniopasivos: '',  
  ...
  catrecibosestatusCat: Catrecibosestatus[];
  catrecibostiposCat: Catrecibostipos[];
  catfondospresupuestalesCat: Catfondospresupuestales[];
  catquincenaactiva:Catquincena;
  
  record_id_personal:number;
  record_id_personal_beneficiario:number;
  ...
  record_personal:any;
  record_personal_beneficiario:any;
  ...
  constructor(
    ...
    private catrecibosestatusSvc: CatrecibosestatusService,
    private catfondospresupuestalesSvc: CatfondospresupuestalesService,
    private catrecibostiposSvc: CatrecibostiposService,
    ...
    this.catrecibosestatusSvc.getCatalogo().subscribe(resp => {
      this.catrecibosestatusCat = resp;
    });
    this.catfondospresupuestalesSvc.getCatalogo().subscribe(resp => {
      this.catfondospresupuestalesCat = resp;
    });
    this.catrecibostiposSvc.getCatalogo().subscribe(resp => {
      this.catrecibostiposCat = resp;
    });
  ...
  newRecord(): Recibos {
    return {
      id: 0, id_personal: 0, numero: "Por definir", fecha: null, anio: '', id_catrecibosestatus: 3,
      id_catrecibostipos: 0, id_catfondopresupuestal: 0, 
      id_catquincena_ini: 0, id_catquincena_fin: 0, observaciones: '',  
      id_catquincena_aplicacion: 0, id_plazas: 0, clavecobro: '',  
      ignorarempparapasivos: 0, ignorarreciboparadecanual: 0,   
  ...
  ngOnInit(): void {
    ...
    modal.recibosService.add(modal);
  ...
  ngOnDestroy(): void {
    this.recibosService.remove(this.id); //idModal
  ...
  async submitAction(admin) {
    ...
    if (this.actionForm.toUpperCase() !== "VER") {
      ...
      await this.isLoadingService.add(
        this.recibosService.setRecord(this.record, this.actionForm).subscribe(async resp => {
  ...
  open(idItem: string, accion: string): void {
    ...
    this.tituloForm =  "Captura de recibos - " 
        + titulosModal[accion] 
        + " registro";
    ...
    this.record_personal_beneficiario="";
    if(this.id_personal_beneficiario){ this.id_personal_beneficiario.clear();this.id_personal_beneficiario.close();}
    ...
    if (idItem == "0") {
      ...
      this.recibosService.getRecord(idItem).subscribe(async resp => {
        this.record = resp;
        ...
        this.personalSvc.getRecord(this.record.id_personal_beneficiario).subscribe(resp => {
          if(resp!=null){
            this.record_personal_beneficiario =resp.numeemp + " - "
              +  resp.nombre + " " + resp.apellidopaterno
              + " " + resp.apellidomaterno + " - " + resp.curp;

            if(this.id_personal_beneficiario){
              //this.id_personal_beneficiario.initialValue = this.record_personal_beneficiario;
              this.id_personal_beneficiario.searchInput.nativeElement.value = this.record_personal_beneficiario;
              this.record.id_personal_beneficiario=resp.id;
            }
          }
        })
      });
    ...
    this.basicModalRecibos.show();
  }

  // close modal
  close(): void {
    this.basicModalRecibos.hide();
    if (this.actionForm.toUpperCase() != "VER") {
